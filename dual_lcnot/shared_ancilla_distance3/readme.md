# Shared-ancilla dual LCNOT: target-Z readout

Shared-ancilla dual LCNOT target-Z readout simulation with fixed-zero logical inputs. Observable 0 records bottom-target Z; observable 1 records top-target Z.
...
```sh
.venv/bin/python scripts/run_simulation.py dual_lcnot/shared_ancilla_distance3/target_z.stim --p-link 0.01 --check-only
```
